...
# A reliable packet to detect lost connections
# (internal use only)
...
# S2C: Header(1), OfflineMesageID(16), server GUID(8), mtu(2), doSecurity(1 bit), handshakeAnswer
# (if do security is true)
...
# Second byte indicates type. Used currently for NAT punchthrough for receiver port advertisement.
# See ID_NAT_ADVERTISE_RECIPIENT_PORT
...
# RakPeerInterface::Receive() you will get ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS. The message will be 5 bytes
# long, and bytes 1-4 inclusive will contain a number in native order containing a number that identifies this message.
# This number will be returned by RakPeerInterface::Send() or RakPeerInterface::SendList(). ID_SND_RECEIPT_ACKED means
# that the message arrived
ID_SND_RECEIPT_ACKED = 0x0e

# If RakPeerInterface::Send() is called where PacketReliability contains UNRELIABLE_WITH_ACK_RECEIPT, then on a later
# call to RakPeerInterface::Receive() you will get ID_SND_RECEIPT_ACKED or ID_SND_RECEIPT_LOSS. The message will be
# 5 bytes long, and bytes 1-4 inclusive will contain a number in native order containing a number that identifies this
# message. This number will be returned by RakPeerInterface::Send() or RakPeerInterface::SendList().
# ID_SND_RECEIPT_LOSS means that an ack for the message did not arrive (it may or may not have been delivered,
# probably not). On disconnect or shutdown, you will not get ID_SND_RECEIPT_LOSS for unsent messages, you should
...
# RakPeer - Reliable packets cannot be delivered to the system specified in Packet::systemAddress.  The connection to
# that system has been closed.
ID_CONNECTION_LOST = 0x16

# RakPeer - We are banned from the system we attempted to connect to.
ID_CONNECTION_BANNED = 0x17

# RakPeer - The remote system is using a password and has refused our connection because we did not set the correct
# password.
...
# RakPeer - The sizeof(RakNetTime) bytes following this byte represent a value which is automatically modified by the
# difference in system times between the sender and the recipient. Requires that you call SetOccasionalPing.
ID_TIMESTAMP = 0x1b

# RakPeer - Pong from an unconnected system.  First byte is ID_UNCONNECTED_PONG, second sizeof(RakNet::TimeMS) bytes
# is the ping, following bytes is system specific enumeration data. Read using bitstreams
ID_UNCONNECTED_PONG = 0x1c

# RakPeer - Inform a remote system of our IP/Port. On the recipient, all data past ID_ADVERTISE_SYSTEM is whatever was
...
# partTotal (unsigned int), partLength (unsigned int), first part data (length <= MAX_MTU_SIZE). See the three
# parameters partCount, partTotal and partLength in OnFileProgress in FileListTransferCBInterface.h
...
# NATPunchthrough plugin: Destination system is not responding to ID_NAT_GET_MOST_RECENT_PORT. Possibly the plugin is
# not installed. Bytes starting at offset 1 contains the RakNetGUID  destination field of
...
# Possibly the plugin is not installed. Bytes starting at offset 1 contains the RakNetGUID destination field of
# NatPunchthroughClient::OpenNAT().
...
# packet::guid contains the destination field of NatPunchthroughClient::OpenNAT(). Byte 1 contains 1 if you are
# the sender, 0 if not.
ID_NAT_PUNCHTHROUGH_FAILED = 0x42

# NATPunchthrough plugin: Punchthrough succeeded. See packet::systemAddress and packet::guid. Byte 1 contains 1 if you
# are the sender, 0 if not. You can now use RakPeer::Connect() or other calls to communicate with this system.
...
# A remote system (not necessarily the host) called FullyConnectedMesh2::StartVerifiedJoin() with our system as the
# client
...
# \note If you fail to connect to a system, this does not automatically mean you will get ID_FCM2_VERIFIED_JOIN_FAILED
# as that system may have been shutting down from the host too
...
# RakPeerInterface::CloseConnection() was automatically called for all systems connected due to
...
# \note Server normally sends us this message, however if connection to the server was lost, message will be returned
# locally
# \note Only the designated client gets this message
ID_FCM2_VERIFIED_JOIN_FAILED = 0x59

# The system that called StartVerifiedJoin() got ID_FCM2_VERIFIED_JOIN_CAPABLE from the client and then called
# RespondOnVerifiedJoinCapable() with true
...
# The system that called StartVerifiedJoin() got ID_FCM2_VERIFIED_JOIN_CAPABLE from the client and then called
# RespondOnVerifiedJoinCapable() with false
...
# Team balancer plugin informing you of your team. Byte 1 contains the team you requested to join. Following bytes
# contain NetworkID of which member.
...
# The password we used to challenge the other system passed, meaning the other system has called
# TwoWayAuthentication::AddPassword() with the same password we passed to TwoWayAuthentication::Challenge()
...
# If the other system must pass the challenge to stay connected, you should call RakPeer::CloseConnection() to
# terminate the connection to the other system.
...
# The other system did not respond within a timeout threshhold. Either the other system is not running the plugin or
# the other system was blocking on some operation for a long time.
# You can read the identifier used to challenge as follows:
# RakNet::BitStream bs(packet->data, packet->length, false);
# bs.IgnoreBytes(sizeof(MessageID));
# RakNet::RakString password; bs.Read(password);
